...
class ProgramBatchController: UIViewController {
    ...
    @IBAction func tapContinue(_ sender: Any) {
        if let viewController = UIStoryboard(name: Storyboards.Deliverables.rawValue , bundle: nil).instantiateViewController(withIdentifier: Views.DeliverablesParentMentor.rawValue) as? DeliverablesTeckersController {
                if let navigator = navigationController {
                    navigator.setNavigationBarHidden(false, animated: true)
//                    TODO: GET TECKERS BY THIS BATCH
                    BatchService.getTeckers(by: batch?.id ?? "", onSuccess: { (teckers) in
                        for tecker in teckers{
                            print("tecker: \(tecker.name), \(tecker.teckerId), \(String(describing: tecker.imageURL)) ")
                        }
                        viewController.teckers = teckers
                    }) { (error) in
                        print(error)
                    }
